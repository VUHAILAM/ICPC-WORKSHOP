...
void pull(node_t<num_t>* x) {
  ...
  x->sum = sum(x->l) + (ll)x->key + sum(x->r);
}
template<typename num_t>
...
int main() {
  ...
  int x;
  int p, q;
  node_t<int> *rt[2], *a0, *b0, *c0, *a1, *b1, *c1;
  int cnt = 1, id;
  int L0, R0, L1, R1;
  do {
    printf("Swapper %d:\n", cnt++);
    ...
    for (int i = 0; i < n; i++) {
      scanf("%d", &x);
      id = i % 2;
      rt[id] = join(rt[id], new node_t<int>(x));
    }
    for (int i = 0; i < m; i++) {
      b0 = b1 = 0;
      //cerr << "before\n"; trace(rt[0]); trace(rt[1]);
      scanf("%d %d %d", &x, &p, &q);
      p--, q--;
      if (p < q or p % 2 == 0) {
        L0 = (p + 1) / 2, R0 = q / 2;
        split(rt[0], a0, b0, c0, L0, R0);
        //debug2(L0, R0); trace(b0);
      }
      if (p < q or q % 2 == 1) {
        L1 = p / 2, R1 = (q - 1) / 2;
        split(rt[1], a1, b1, c1, L1, R1);
        //debug2(L1, R1); trace(b1);
      }
      if (x == 1) {
        rt[0] = join(join(a0, b1), c0);
        rt[1] = join(join(a1, b0), c1);
      } else {
        printf("%lld\n", sum(b0) + sum(b1));
        if (b0) rt[0] = join(join(a0, b0), c0);
        if (b1) rt[1] = join(join(a1, b1), c1);
      }
      //cerr << "after\n"; trace(rt[0]); trace(rt[1]);
    }
    scanf("%d %d", &n, &m);
